...
// Funciones NIT
function validarNit(nit: string): { valido: boolean; nit: string; tipo: string; mensaje: string; digitoVerificacion?: number } {
...
function formatearNit(nit: string): string {
...
function limpiarNit(nit: string): string {
	return nit.replace(/[^0-9]/g, '');
}

function calcularDigitoVerificacion(nitBase: string): { nitBase: string; digitoVerificacion: number; nitCompleto: string } {
...
async function obtenerTrm(that: IExecuteFunctions): Promise<IDataObject> {
	try {
...
async function obtenerUvt(that: IExecuteFunctions): Promise<IDataObject> {
...
function convertirUvtPesos(uvt: number, valorUvt: number): IDataObject {
	return {
		uvt,
		valorUvt,
...
function convertirPesosUvt(pesos: number, valorUvt: number): IDataObject {
	return {
		pesos,
		valorUvt,
...
export class DianColombia implements INodeType {
			...
			try {
				...
				if (resource === 'nit') {
					if (operation === 'validar') {
						const nit = this.getNodeParameter('nit', i) as string;
						...
						const nitBase = this.getNodeParameter('nitBase', i) as string;
				...
				// Indicadores
					...
					if (operation === 'trm') {
						...
						const montoUvt = this.getNodeParameter('montoUvt', i) as number;
						const uvt = await obtenerUvt(this);
						result = convertirUvtPesos(montoUvt, (uvt as any).valor);
					} else if (operation === 'pesos_uvt') {
						const montoPesos = this.getNodeParameter('montoPesos', i) as number;
						const uvt = await obtenerUvt(this);
						result = convertirPesosUvt(montoPesos, (uvt as any).valor);
					}
				}
